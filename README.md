...
| Agent 框架 | LangChain（仅用于消息格式与流式）+ DashScope 原生 Embedding     |
...
1. **已知信息录入** — 手动输入汇总信息（表已建，待前端接入）
2. **连线备注编辑** — 双击连线或选中后在面板编辑 label（后端 API 已支持，纯前端工作）
3. **技能管理** — 预置推理技能（动机分析、时间线梳理等）+ 用户自定义
4. **时间线自动生成** — 利用 Agent 从便签/文档中自动提取时间线事件
...
- **2025-06-04**：完成文档上传功能，集成 text-embedding-v4 向量化，前端增加文档 Tab。
- 先前版本：实现便签墙、时间线、Agent 流式对话、RAG 检索、案件管理等核心功能。